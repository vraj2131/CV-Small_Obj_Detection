...
# 1. Compute loss (now returns total_loss and a 3-element tensor of [lbox, lobj, lcls])
                loss, losses = compute_loss(pred, targets.to(device))

# 2. Unpack the individual losses
                lbox, lobj, lcls = losses.unbind(0)

# 3. If using the super-resolution branch, add its loss
                if opt.super:
...
# 4. Build loss_items as a 1D tensor [lbox, lobj, lcls, total_loss]
