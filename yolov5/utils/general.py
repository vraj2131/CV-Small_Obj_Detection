...
LOGGING_NAME = "yolov5"  # or whatever default you were using

def set_logging(name=LOGGING_NAME, verbose=True):
    """Configures logging; coercing `name` to str so dictConfig never sees an int."""
    ...
    logging.config.dictConfig({
        ...
        "root": {                       # ensure root logger also uses our handler
            "level":    level,
            "handlers": [name_str]
        }
    })
...
# now get your global LOGGER
LOGGER = logging.getLogger(str(LOGGING_NAME))

# Windows‐safe emoji support, unchanged
if platform.system() == "Windows":
    from utils.general import emojis
    for fn in (LOGGER.info, LOGGER.warning):
        setattr(LOGGER, fn.__name__, lambda msg, fn=fn: fn(emojis(msg)))
